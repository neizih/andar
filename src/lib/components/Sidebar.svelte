...
<script>
  ...
  function handleClickOutside(event) {
    if (isOpen && !event.target.closest('.sidebar') && !event.target.closest('.menu-button') && !event.target.closest('.close-button')) {
      isOpen = false;
    }
  }
...
<!-- Mobile menu button (left side - hamburger) -->
<button
...
<!-- Mobile close button (right side - X) -->
{#if isOpen}
  <button
    class="close-button lg:hidden fixed top-4 right-4 z-50 p-3 rounded-xl bg-white shadow-xl border border-gray-200 hover:shadow-2xl transition-all duration-200 backdrop-blur-sm"
...
<style>
  ...
  /* Smooth entrance animation for close button */
  .close-button {
